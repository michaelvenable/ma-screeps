...
/**
 * Adds towers to the in-memory map for all occupied rooms. The towers are placed near controllers.
 *
 * Usage:
 * require('cli').run('construction', 'plan-towers-near-controllers');
 */
function run() {
    ...
    for (let name in Game.rooms) {
        ...
        if (map === undefined) {
            throw new Error("You must run construction.commands.create-maps before running construction.commands.plan-towers-near-controllers.");
        }

        let buildList = Memory.architect.buildLists[name] || [];

        strategies.buildTowersNearControllers.run(room, map, buildList);
