...
/**
 * Adds links to the in-memory map for all occupied rooms. The links are placed near energy sources.
 *
 * Usage:
 * require('cli').run('construction', 'plan-links-near-energy-sources');
 */
function run() {
    ...
    for (let name in Game.rooms) {
        ...
        strategies.buildLinksNearEnergySources.run(room, map, buildList);
    ...
    worklist.add('construction', 'plan-links-near-spawns', { ticksFromNow: 3 });
}
